...
class CarControllerParams:
  STEER_MAX = 300  # GM 제한값은 300. carcontroller에서 LKA 출력(토크)생성에 사용되는 값.
  STEER_STEP = 4  # kans: 활성프레임 주파수(콤마기본값은 33hz) 부드러운 조향을 위해 25Hz로 임의수정(2,3,4.각자선택).
  INACTIVE_STEER_STEP = 10  # Inactive control frames per command (10hz)
  STEER_DELTA_UP = 7 # 높은값에 EPS(전동PwrStr)약화가 관측되었으므로 관찰,조정 필요.
  STEER_DELTA_DOWN = 9 # 너무 빨리 풀면 촐랑될 수 있으니 관찰,조정 필요.
  STEER_DRIVER_ALLOWANCE = 65
  STEER_DRIVER_MULTIPLIER = 4
  STEER_DRIVER_FACTOR = 100
  NEAR_STOP_BRAKE_PHASE = 0.13  # m/s. 낮추면 정지시 브레이크충격 완화에 도움됨(0.1 ~ 0.15이 적당) 
  SNG_INTERCEPTOR_GAS = 18. / 255.
  SNG_TIME = 30 # SNG_INTERCEPTOR_GAS 도달 frame값
