...
PEDAL_MSG = 0x201 #pedal_interceptor finger 513
CAM_MSG = 0x320  # AEBCmd
                 # TODO: Is this always linked to camera presence?
ACCELERATOR_POS_MSG = 0xbe #regen_paddle finger 190
...
class CarInterface(CarInterfaceBase):
  ...
  def _get_params(ret: structs.CarParams, candidate, fingerprint, car_fw, experimental_long, docs) -> structs.CarParams:
    ...
    if candidate in (CAMERA_ACC_CAR | SDGM_CAR):
      ...
      ret.longitudinalTuning.kiV = [2.0]
      ...
      ret.radarUnavailable = False # kans RADAR_HEADER_MSG not in fingerprint[CanBus.OBSTACLE] and not docs
    ...
    if candidate == CAR.CHEVROLET_VOLT:
      ...
      #GM, Pedal_Long
      if ret.enableGasInterceptor:
        # ACC Bolts use pedal for full longitudinal control, not just Stop&Go
    ...
    elif candidate == CAR.CADILLAC_CT6_CC: # kans: add CT6_CC
      ret.steerActuatorDelay = 0.2
      CarInterfaceBase.configure_torque_tune(candidate, ret.lateralTuning)
    ...
    if ret.enableGasInterceptor:
      ...
      if candidate in CC_ONLY_CAR:
        ...
        ret.longitudinalTuning.kiBP = [0.]
        ret.longitudinalTuning.kiV = [0.07]
        ret.longitudinalTuning.kf = 0.23
      ...
      if experimental_long:
        ret.openpilotLongitudinalControl = True
        ret.flags |= GMFlags.CC_LONG.value
      ret.radarUnavailable = False if candidate in CAR.CADILLAC_CT6_CC else True # kans
      ret.experimentalLongitudinalAvailable = False
      ...
      ret.longitudinalTuning.kiBP = [0.]
      ret.longitudinalTuning.kiV = [0.3]  # set lower end to 0 since we can't drive below that speed
